...
class Question extends React.Component {
	...
	renderButton() {
		...
		if (loading) {
			...
			return (
				<button className="btn btn-lg" onClick={onClickNext}>
					{counter < quizLength - 1
						? "NEXT QUESTION"
						: "GO TO RESULTS"}
				</button>
			);
				...
				<React.Fragment>
					...
					>
						<div className="d-flex flex-column align-items-center">
							<p className="h4 my-3">
								QUESTION {counter + 1} of {quizLength}
							</p>
							<div className="mx-3">
								{decodeHtml(trivia[counter].question)}
							</div>
							<div className="list-group my-4">
								<Answers />
							</div>
							<div className="mb-4">{this.renderButton()}</div>
						</div>
					</CSSTransition>
				</React.Fragment>
			);
