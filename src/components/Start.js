...
class Start extends React.Component {
	...
	renderCategories() {
		...
		if (categories.length === 0) {
			return (
				<option className="text-dark" value="loading" key="loading">
					LOADING...
				</option>
			);
		} else {
			if (categories[0].error) {
				return (
					<option className="text-dark" value="error" key="error">
						Server Error
					</option>
				);
			} else {
				return [
					<option className="text-dark" value="X" key="X">
						CATEGORIES...
					</option>
				].concat(
					categories
		...
		if (!loading) {
			return (
				<button
					className={
						"btn btn-lg" +
						(selectedCategory !== null && selectedCategory !== "X"
							? ""
							: " btn-secondary")
					}
					onClick={onClick}
				...
				>
					<div className="row d-flex justify-content-center align-items-around mw-25">
						<p
							...
							className="col-12 h5 text-center pt-3 mb-0"
						>
							{this.type()}
							&nbsp;
						</p>
